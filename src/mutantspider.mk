...
#
#	$1 the file name to "sanitize" (make it a legal C token)
#
#		rules:
#			X	-> XX
#			.	-> X_
#
ms.sanitize_rez_name=rezString_$(subst .,X_,$(subst X,XX,$(1)))

#
# $1 file name of file to be treated as a resource
#
...
$(call ms.resrc_to_auto_gen,$(1)): $(1) | $(dir $(call ms.resrc_to_auto_gen,$(1)))dir.stamp
	...
	@echo "};" >> $$@
	@echo "const unsigned char* $(call ms.sanitize_rez_name,$(1)) = &$(call ms.sanitize_rez_name,$(1))_[0];" >> $$@
	@printf "size_t $(call ms.sanitize_rez_name,$(1))_sz = " >> $$@
	@printf "`stat -f %z $(1)`;\n" >> $$@
	@echo "}" >> $$@
...
ms.rez_decl=$(foreach rez,$(RESOURCES),extern unsigned char* $(call ms.sanitize_rez_name,$(rez)); extern size_t $(call ms.sanitize_rez_name,$(rez))_sz;)
ms.rez_code=$(foreach rez,$(RESOURCES),g_resource_map.insert(std::make_pair(\"$(notdir $(rez))\",std::make_pair($(call ms.sanitize_rez_name,$(rez)),$(call ms.sanitize_rez_name,$(rez))_sz)));)
ms.rez_files=$(foreach rez,$(RESOURCES),$(call ms.resrc_to_auto_gen,$(rez)))

$(ms.INTERMEDIATE_DIR)/auto_gen/resource_list.cpp: $(RESOURCES)
	@echo "// AUTO-GENERATED by mutantspider.mk, based on the value of (RESOURCES)" > $@
	@echo "// DO NOT EDIT" >> $@
	@echo "" >> $@
	@echo "#include <map>" >> $@
	...
	@echo "" >> $@
	@echo "$(ms.rez_decl)" >> $@
	@echo "std::map<const char*, std::pair<const unsigned char*, size_t> > g_resource_map;" >> $@
	@echo "" >> $@
	@echo "void init_resource_map()" >> $@
	@echo "{" >> $@
	@echo "	$(ms.rez_code)" >> $@
	@echo "}" >> $@
	@echo "" >> $@
	@echo "const unsigned char* get_named_resource(const char* key)" >> $@
	@echo "{" >> $@
	@echo "	auto it = g_resource_map.find(key);" >> $@
	@echo "	return it == g_resource_map.end() ? 0 : it->second.first;" >> $@
	@echo "}" >> $@
	@echo "" >> $@
	@echo "size_t get_named_resource_size(const char* key)" >> $@
	@echo "{" >> $@
	@echo "	auto it = g_resource_map.find(key);" >> $@
	@echo "	return it == g_resource_map.end() ? 0 : it->second.second;" >> $@
	@echo "}" >> $@
	@echo "" >> $@
	@echo "}" >> $@
