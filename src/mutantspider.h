	...
	*/
	...
	namespace mutantspider {
		...
		protected:
			...
			public:
				ab_data()
					: m_ptr(0), m_len(0)
				{}
				ab_data(const void* data, size_t len)
					: m_ptr(data), m_len(len)
				{}
				ab_data(const ab_data& rhs)
					: m_ptr(rhs.m_ptr),
					  m_len(rhs.m_len)
				{
					// hack, for now just stealing the data
					const_cast<ab_data&>(rhs).m_ptr = 0;
				}
				~ab_data()
				{
					if (m_ptr)
		...
		public:
			...
			size_t ByteLength() { return m_ab_data.m_ptr ? m_ab_data.m_len : 0; }
			const void* Map() { return m_ab_data.m_ptr; }
			void Unmap() {}
		};
#endif
